...
class Config:
    ...
    # source prompt to use during inversion
    source_prompt: str = ""
...
def create_splats_with_optimizers(
    ...
    if start_ckpt is not None:
        print(f"Loading starting checkpoint from {start_ckpt}...")
        ckpt = torch.load(start_ckpt, map_location="cpu")
        splats_dict = ckpt["splats"]

        # Distribute the GSs to different ranks
        points = splats_dict["means"][world_rank::world_size]
        scales = splats_dict["scales"][world_rank::world_size]
        quats = splats_dict["quats"][world_rank::world_size]
        opacities = splats_dict["opacities"][world_rank::world_size]
        ...
        if "sh0" in splats_dict:
            sh0 = splats_dict["sh0"][world_rank::world_size]
            shN = splats_dict["shN"][world_rank::world_size]
            params.append(("sh0", torch.nn.Parameter(sh0), sh0_lr))
            params.append(("shN", torch.nn.Parameter(shN), shN_lr))
        elif "features" in splats_dict:
            features = splats_dict["features"][world_rank::world_size]
            colors = splats_dict["colors"][world_rank::world_size]
...
class Runner:
    ...
    @torch.no_grad()
    def render_and_edit_ddim(self, loader):
    ...
    def render_and_edit_ddim_with_depth(self, loader):
        ...
        for i, data in tqdm.tqdm(
            ...
            for j, img_id in enumerate(img_ids):
                ...
                # alpha = 0.5 + 0.25 * torch.rand(1).item()
                # original_image_chw = pixels[j]
                # edited = alpha * edited + (1 - alpha) * colors.to(edited.device)
    ...
    @torch.no_grad()
    def render_and_edit_ddim_with_depth_and_multiview_3d_projection(self, loader):
        cfg = self.cfg
        device = self.device
        ...
        import propagate_results_to_3d
        ...
        ddim_editor = propagate_results_to_3d.DDIMInversionEditorDepthMultiview(
            device=device
        )
        cached_transformed_imgs = ddim_editor.run_edits(
            idx2image=idx2image,
    ...
    def train(self):
        ...
        # cached_edited_imgs = (
        #     self.render_and_edit_ddim_with_depth_and_multiview_3d_projection(
        #         trainloader
        #     )
        # )
        ...
        for step in pbar:
            ...
            pixels = []
            for img_id in data["image_id"].tolist():
                pixels.append(cached_edited_imgs[img_id])

            pixels = torch.stack(pixels, dim=0).to(device)
