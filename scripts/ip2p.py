...
try:
    ...
    CONSOLE.print(
        r"or [yellow]pip install -e .\[gen][/yellow] if installing from source."
    )
    sys.exit(1)
...
class InstructPix2Pix(nn.Module):
    ...
    def __init__(
        ...
        pipe.scheduler = DDIMScheduler.from_pretrained(
            DDIM_SOURCE, subfolder="scheduler"
        )
    ...
    def latents_to_img(
        self, latents: Float[Tensor, "BS 4 H W"]
    ...
    def imgs_to_latent(
        self, imgs: Float[Tensor, "BS 3 H W"]
        ...
        posterior = self.auto_encoder.encode(
            imgs.to(self.auto_encoder.dtype)
        ).latent_dist
    ...
    def prepare_image_latents(
        ...
        image_latents = self.auto_encoder.encode(
            imgs.to(self.auto_encoder.dtype)
        ).latent_dist.mode()
