...
class ClipSimilarity(nn.Module):
    def __init__(self, device="cpu", name: str = "ViT-L/14"):
        super().__init__()
    ...
    def encode_image(self, image):  # Input images in range [0, 1].
        image = image.to(self.device)  # Ensure image is on the correct device
        image = F.interpolate(
    ...
    def forward(self, image_0, image_1, text_0, text_1):
        ...
        v_img = (image_features_1 - image_features_0).float()
        v_text = (text_features_1 - text_features_0).float()
        sim_direction = F.cosine_similarity(v_img, v_text, dim=-1, eps=1e-8)
