...
def compute_attn(
    attn, query, key, value, video_length, ref_frame_index, attention_mask
):
...
class CrossViewAttnProcessor:
    ...
    def __call__(
        ...
        if not is_cross_attention:
            ...
            hidden_states_ref0 = compute_attn(
                attn, query, key, value, video_length, ref0_frame_index, attention_mask
            )
            hidden_states_ref1 = compute_attn(
                attn, query, key, value, video_length, ref1_frame_index, attention_mask
            )
            hidden_states_ref2 = compute_attn(
                attn, query, key, value, video_length, ref2_frame_index, attention_mask
            )
